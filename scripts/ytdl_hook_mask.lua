...
--
-- To customize configuration place ytdl_hook_mask.conf template into
-- dir ~/.config/mpv/script-opts/ [~/.config/mpv/lua-settings/] and edit
--
-- Place script into ~/.config/mpv/scripts/ for autoload and disable built-in ytdl
-- with command line options --no-ytdl or by addiung ytdl=no to ~/.config/mpv/mpv.conf
--
...
-- log active config
mp.msg.verbose('cfg = ' .. utils.to_string(o))
...
local function ytdl_hook()
    ...
    if is_blacklisted(url) then
        msg.verbose('no action - url is blacklisted')
        return
    end
    run_ytdl_hook(url)
end

-- hook ol-load/on-load-fail
mp.add_hook(event, 10, ytdl_hook)
